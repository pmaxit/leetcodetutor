...
![ZoomInfo](https://files.hellointerview.com/build-assets/PRODUCTION/_next/static/media/zoominfo.06861-vl3s5hh.svg?dpl=a5eaff0b0b1e4190375aeda4ce53625448283294)
...
6.  If the Crawl-delay time has not passed since the last crawl, use ChangeMessageVisibility to extend the visibility timeout and defer reprocessing.
...
3.  **How to handle large files**: Some websites have very large files that we may not want to download. We can send an HTTP HEAD request first to check the Content-Length header and skip files that exceed a threshold (say 10MB) without downloading the full content.
...
[Next: Ad Click Aggregator](/learn/system-design/problem-breakdowns/ad-click-aggregator)
...
[• 1 year ago](#comment-clwi993er001b1nmcy1ucdrpz)

I've spent some time thinking about scheduling for this system design since doing this in my mock and I think it's also useful to at least keep track of the number of URLs currently enqueued for a given domain.

The reason is, if we enqueue a lot of URLs for a single domain at the same time (e.g. we found 100 internal links on a webpage), if we write them back to the SQS queue with the fixed crawl delay, in the worst case we'll potentially be repeatedly dequeuing them simultaneously, only be able to handle 1 of them and have to re-queue all the others, up to (n\*(n+1)/2) times. I.e. to handle those 100 URLs, it could take 5050 enqueue and 5050 dequeue operations.

If we tracked how many were pending on the queue for each domain we could re-queue with a delay of (queuedCount + 1)\*crawlDelay. The problem is with SQS that there's a maximum delay, so this only works up to a point.

You could stored enqueued URLs in a DB and enqueue just the domain in SQS with the delay. When the crawler dequeues the domain, it grabs a URL for that domain from the DB (LIMIT 1) and crawls it, updated the URL and last crawl time and then re-enqueues the domain with the correct delay. Something feels a bit fragile with this scheme though. Maybe the fact we don't have an atomic operation to handle the ack and the re-enqueue.

I think there's an argument for building a more custom queue/scheduler on top of a database. To minimize total crawl time for the entire graph, we need to prioritize domains which have longest expected total time to crawl (number of enqueued URLs \* crawl delay), which required more logic than is possible with a simple queue queue. The logic is sort of similar to Leetcode problems like Task Scheduler and Find Minimum Time to Finish All Jobs.
...
[• 1 year ago](#comment-clwi9wz56001exgwvhlgpkwu3)

Wow, made a lot of typos there. Well, I'll just leave them instead of repeatedly deleting and re-posting. Hint: edit button please 🙂

Also to add: for prioritization, in addition to cost to crawl every URL in a domain, we may also have a concept of quality too. For example the higher the URL depth the lower the URL quality. Adding a bunch of very deep URLs to a domain could increase its priority less than URLs found close to the initial seed URLs. The obvious tradeoff is it's complicated and more expensive to compute all that. And I guess makes no difference if we intend to just keep crawling until we have crawled (or hit the retry limit for) every single URL. But if we want to crawl as much useful data in some time-boxed window which the potential we might terminate with some URLs still pending, it could be important.
...
ExtremeAzureHornet561

[• 1 year ago](#comment-clwiczltc0026u7bknk60oud9)

Can we just use separate queue for each domain and dynamically adjust the number of workers assigned to each domain queue based on the queue size and processing time.
...
ReligiousBlackCricket961

Top 5%

Top 5%

[• 1 year ago](#comment-clwjkf9l4000xulfgd2vbiieh)

Separate queue for each domain will not scale
...
[• 1 year ago](#comment-clwieq8du002qu7bkusuza3eg)

Yeah, I thought about queue per domain too. I think it works as long as you have lightweight queues.

Show more

0

![Aparna K](https://lh3.googleusercontent.com/a/ACg8ocLfF6GdZQiyJgUTfWbABxvTgXyDdcTBFa8TAPvwSwpBXtBxuQ=s96-c)

Aparna K

[• 1 year ago](#comment-cm06ewoij0015amnxjdc0yfqf)

If the frontier queue is kafka, using domain name as partition key would also route to the same crawler machine.
...
[• 1 year ago](#comment-clwjlmb1g0011efda2oui3s8n)

We can also use a load balancer that supports session persistence to ensure that requests from the same domain are always routed to the same crawler machine.
...
[• 1 year ago](#comment-clwid0wrq0028u7bkxni524vg)

Also Implementing a priority queue to prioritize domains with fewer pages or higher priority tasks will be helpful as well
...
[• 1 year ago](#comment-clwjgpy4d0003efdao9yuqduf)

Priority queue is nice, particularly for a search engine implementation. When you're rescraping regularly, you'll want to be smart and prioritize. Less emphasis on a one-time scrape.
...
[• 6 months ago](#comment-cmh1v9rdg04qm08ado7w64gl3)

Why can't we use hostname as key then each url will get pulled by worker one by one
...
ElectricEmeraldBoa808

Premium

Premium

[• 1 year ago](#comment-cm8oqfjne00mqox0pd1dbjrjj)

Hi Evan, I was asked a variation of this question ([https://www.hellointerview.com/premium/questions/cm5gdl97c0009356qbi9mp88b?company=Meta&level=SENIOR](https://www.hellointerview.com/premium/questions/cm5gdl97c0009356qbi9mp88b?company=Meta&level=SENIOR)). It comes with additional constraints:

1.  10,000 machines
2.  Can't use external services (like S3 or Dynamo)
3.  Internal network bandwidth is limited

The interviewer was not particularly interested in rate limiting, robot.txt, or other topics in this post. These constraints totally got me because I haven't practiced much outside the context of distributed services.

These were my responses:

1.  Use Kubernetes to manage the crawling/storage clusters.
2.  Choose Cassandra to store metadata because the access pattern is write-heavy.

However, I didn't address how to fully utilize the 10k machines or how to merge the files (because I don't know how to use local machines to mimic S3).

Wondering how you would address this question variation.
...
[• 1 year ago](#comment-cmaa17x7502o7ad07p84sfkva)

Very interesting constraint -- you would probably need to do more processing on the Crawler itself vs. splitting up services as is done here to avoid internal bandwidth issues. You could avoid using s3 entirely and just parse in memory directly after retrieval -- usually a worse idea, but might make sense with this variant.

Also thinking that the Crawler might use some kind of in-memory queue in addition to Kafka to save significant bandwidth. We would only go back to Kafka at e.g. depth % N or if the internal queue starts getting large.
...
[• 1 year ago](#comment-cm9p0t74e01rvad083k1eq42j)

that's an interesting variant, did you have time to analyze it a bit more? Did you pass this interview?
...
[• 1 year ago](#comment-cm9ramcu000k9ad089nut0m50)

Unfortunately no. I did not pass the interview and got a downgraded offer.

I still don't know the right design for these constraints.
...
[• 5 months ago](#comment-cmhy3qp8l00gj08adzvsvyepu)

It looks like a "Create your own HDFS" problem, especially if you can't use an external blob storage service.
...
[• 18 days ago](#comment-cmo3295a80m9h08adtgobxpos)

Maybe. But I think testing web crawler + HDFS design in an hour is a very ambitious challenge for senior level.
...
[• 1 year ago](#comment-clwoa0ftg0031irp05kixwobk)

Why don't we push s3 url to parsing queue? this way parsing worker don't have to query metadata db and can go to s3 directly.
...
[• 1 year ago](#comment-clws277oz0095unw1m1mor666)
...
[• 11 months ago](#comment-cmbecq0db0366ad08jkvmvapf)

I think it's also fine to push the url record (url, id, s3 url) to the queue.
...
[• 1 year ago](#comment-cm6yphz3a02338xn7pquq97wc)

I think it is better to have one status field to indicate data flow status, then parser needs to update that in DB
...
[• 1 year ago](#comment-clwixlqpx007h1nmck01l7f9c)

Why do you need rate limiting if you already using crawl delay? Isn't this redundant?
...
[• 1 year ago](#comment-clwjgnw500002ltbbdzke661x)

Yeah, many sites won't have a crawl delay in robots. So we generally adhere to a 1 request per second limit in its absence.
...
InclinedGoldShark552

[• 1 year ago](#comment-clz4c9rld00d35nhc43w729fz)

For websites that have a crawl delay, would it be possible to still use Redis to uphold the rate limit? i.e. bypassing usage of the crawlDelay in the Metadata DB
...
![JC](https://lh3.googleusercontent.com/a/ACg8ocKqbnGgB0guGNr-qF8Nc6MWNCZ0n5tNg7gcfGBb_GcOnrO8oC79BA=s96-c)
...
[• 1 year ago](#comment-cm2xy632k00d213d542dlt8ie)

Hi Evan and thanks for the write-up and excellent video. I have a question about the approach using a sliding window for rate limiting. I was wondering if we might achieve similar results with a potentially simpler setup by leveraging Redis's TTL feature.

In this approach, we could store each domain with a TTL in Redis equal to its crawl-delay (or 1 second if none exists). Before each fetch, a crawler would check if the domain key exists in Redis. If it does, it would return the request to the queue with a slight jitter; if not, the crawler would add the key to Redis with the appropriate TTL and proceed. Redis’s has an atomic operation for confirming the existence of a key, and inserting it at the same time so we wouldn’t run into issues with multiple readers checking for a key; the first reader would insert it, causing the others to see the key.

Would this simpler, TTL-based approach work in your view? Or do you see any potential limitations compared to the sliding window approach?
