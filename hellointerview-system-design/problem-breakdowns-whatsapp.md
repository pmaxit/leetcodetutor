# Design a Messaging App Like WhatsApp | Hello Interview System Design in a Hurry
...
By[Stefan Mai](https://www.linkedin.com/in/stefanmai/)·Updated Feb 15, 2026·
...
**🚗 What is [Whatsapp](https://www.whatsapp.com/)?** Whatsapp is a messaging service that allows users to send and receive encrypted messages and calls from their phones and computers. Whatsapp is famously built on Erlang and renowned for handling high scale with limited engineering and infrastructure outlay.
...
The issue is one of of routing: we need to route messages to the right Chat Servers in order to deliver them. We have a few options here which are discussed in greatest depth in the [Realtime Updates Deep Dive](/learn/system-design/deep-dives/realtime-updates).
...
**Note**: Some readers are concerned about the scalability of Pub/Sub. Beyond the memory constraints (we spoke about those earlier, we don't need much memory!), can we really handle the number of messages? [Canva has a nice benchmark of this](https://www.canva.dev/blog/engineering/realtime-mouse-pointers/) where they supported 100,000 mouse position updates per second on a single Redis host with 27% utilization! Remember that the Pub/Sub is _really_ dumb and just passing messages, so it can be **very** efficient and scalable!
...
*   _If we partition by chat_ the total number of channels in the system is 125 per user (250 chats / 2 since each chat is shared). But each chat server still needs to subscribe to 250 channels per connected user (one for each of that user's chats). When a message is sent, the server publishes to just 1 channel.
*   _If we partition by user_ there's 1 channel per user. Each chat server subscribes to 1 channel per connected user. When a message is sent, the server publishes to 1 channel (the recipient's).
...
*   _If we partition by chat_ the total number of channels is 1 per 100 users (since all 100 share a single chat channel). Each chat server subscribes to 1 channel per connected user. When a message is sent, the server publishes to just 1 channel.
*   _If we partition by user_ there's 1 channel per user. Each chat server subscribes to 1 channel per connected user. When a message is sent, the server has to publish to 99 channels (one for each other participant).
...
[Next: Rate Limiter](/learn/system-design/problem-breakdowns/distributed-rate-limiter)
...
[• 1 year ago](#comment-clxi0gadz00aglua46gbe83nt)
...
[• 1 year ago](#comment-clxi0v4wf008xbvlk2ccqn5s8)
...
[• 1 year ago](#comment-clzyacy5v007ezkz1sd083y76)
...
[• 1 year ago](#comment-clzyb61ju007uqy3fs0hbe0q0)
...
[• 1 year ago](#comment-clzybs4l70085qy3ft3f02may)
...
[• 1 year ago](#comment-cm21ox94z00ekabj6lqsas328)

We can add some id on ingestion and when client gets real time messages from redis , it might find some ids missing based on last message received. It can query db for missed messages. On every reconnection, client can have a db call

Show more

1

![YOUNG M](https://lh3.googleusercontent.com/a/ACg8ocLzxaVG6W1redYjtwgOiNLoHKn4TZd9btWmtvXqRyXSU-e3GQ=s96-c)

YOUNG M

[• 1 year ago](#comment-cm273bvxu00of12hcxsihe0td)
...
[• 3 months ago](#comment-cmk3tvvo901qf08adpkyd0sm9)
...
[• 1 year ago](#comment-clxi5tlh300aniuc6ebosd5kn)
...
[• 1 year ago](#comment-clxifyh6d00bsiuc6u8h5xg2c)
...
[• 1 year ago](#comment-clxsh6bjp00ej252ouysuig1x)
...
[• 1 year ago](#comment-clz07m1uk004khy4syefgcysi)
...
[• 1 year ago](#comment-cm8kyjsc4005d99lgavucog9i)
...
Because it looks like both solutions are very similar which need to: 1 create a topic per users 2 subscribe to the topics of the users who are connected to the chat sever. 3 publish the message to the users' topics 4 send the message to users via the websockets.
...
[• 1 year ago](#comment-cm9lyq4wa00atad0892l65wpn)
...
[• 4 months ago](#comment-cmjx2vo8409h108adnx0rbsys)
...
![Dmitry Oksenchuk](https://lh3.googleusercontent.com/a/ACg8ocLx1XBSSpFxCFmKn-b9YIEqtulWTuTbJY_Yb7PCYiptxts7Kw=s96-c)

Dmitry Oksenchuk

[• 3 months ago](#comment-cmkqpcmg10zj708ad5cc9grbq)

[https://stackoverflow.com/questions/32950503/can-i-have-100s-of-thousands-of-topics-in-a-kafka-cluster](https://stackoverflow.com/questions/32950503/can-i-have-100s-of-thousands-of-topics-in-a-kafka-cluster)

> Update March 2021: With Kafka's new KRaft mode, which entirely removes ZooKeeper from Kafka's architecture, a Kafka cluster can handle millions of topics/partitions. See [https://www.confluent.io/blog/kafka-without-zookeeper-a-sneak-peek/](https://www.confluent.io/blog/kafka-without-zookeeper-a-sneak-peek/) for details.

Show more

2

![Pranav Cool](https://lh3.googleusercontent.com/a/ACg8ocJq0Z1bOXtBc9vfL8PL7xzKyyi_qwYprhVb8E-pjMCOe4DPhg=s96-c)

Pranav Cool

[• 2 months ago](#comment-cmldnudjc05y707adsn15h0nl)
...
[• 11 months ago](#comment-cmaewped800amad07zv91u5ym)
...
[• 3 months ago](#comment-cmkhcdwkj01db08adpqx9v6v1)
...
[• 21 days ago](#comment-cmnyai27a0k0c07adru2s0qz5)

I think even with consistent hashing you would have at-most-once delivery. The hashing information distributed via ZooKeeper would determines _which_ server is responsible for a user. But delivery semantics are depend on _how_ the assigned server interacts with clients.

For live streaming at-most-once is basically reality. You can't force a client to receive a websocket message, they can disconnect at any time.

When a user connects with WSS they would connect to a single server. That server subscribes to the channel. Presumably it's the only server subscribed unless another server has a stale connection listed because the user just lost connection and reconnected. Then the message flow is message service --> Redis consistent hashing --> Redis node with the channel --> broadcast to the O(1) WS servers subscribed --> client. So 1:O(1) fanout, not MxN.

I admit the Redis approach suffers from an added hop from Redis to the WS server. But it also allows the WS layer, which is one of the biggest scaling challenges, to scale horizontally and be mostly stateless. As Stefan suggests, rolling your own consistent hashing scheme and rebalancing connections quickly gets complicated.
...
[• 1 year ago](#comment-cm5bqk89v03j3i96o02n9vjie)

Hi Stefan, The article was good and very helpful, particularly the Redis approach. I have one clarification.
...
[• 1 year ago](#comment-cm5br9fsz03mjv80895bfpzoz)
...
[• 1 year ago](#comment-cm5d0ckcj00o7m9c3fjgbh4zr)

The tradeoff are as follows

1.  Message table - Scatter and Gather: Pros: Storage is optimized - that is the message content is stored only once. Cons: a) Two queries - one to get the messageId from Inbox and other to get content from Messages. More IOPS, query is expensive - it has to execute in multiple shards and merge the results.
    
2.  Message content in Inbox: Pros: Only one query and if the sort key is clientId - then mostly probably one disk seek is enough. It can improve the p99. Cons: Storage is high. Message content is duplicated for all offline members.
    

My take: I will go with storing the message content in Inbox - because the storage cost is less than compute cost (anyway we are going to purge the data) and one query is enough to fetch the message. May be hybrid approach might work - larger message size store it in the Message table and store the content in Inbox for shorter message. But not sure whether this is over engineering.
...
[• 1 year ago](#comment-cm5d0lc8z00omm9c3mrbsr2xo)
...
[• 1 year ago](#comment-cm6yabng501cfun62d0magekr)
...
[• 1 year ago](#comment-cm5d15ozn00pw612z12y0najs)
...
NavalYellowHalibut104

Premium

Premium

[• 9 months ago](#comment-cmdwkxmvr00huad07krpxu148)

Solutions mention a load balancer but doesn't talk about connection limits of the load balancer, only talks about horizontal scale in terms of websocket servers. For example aws nlb supports only 400k connections. Should we scale up load balancers as well then ?
...
[• 3 months ago](#comment-cmkhcq9w701kc08adlvdlar1m)

Hardware load balancers scale into the 10 million range. For connections beyond this, you'd typically use DNS to spread the load (so different users would connect to different load balancers).

If you were actually forced to design this, I'd probably expose the chat servers publicly (i.e. they'd have publicly addressable network adapters and DNS entries). Then you'd have some sort of registry server/service to hand out addresses for the chat server which a new connection should use.

I don't find these type of deep dive questions that common in interviews as they tend to learn more into network operations which are more specialized. Happy to have someone with more experience there weigh in!

Show more

0

![tilak chowdary](https://lh3.googleusercontent.com/a/ACg8ocKuaSp-ot03GcZ44015KbF9YlDXUnY2zvoIQVBCpYP7tlDJTw=s96-c)
...
[• 7 months ago](#comment-cmf8niwkl02nw08adz7zgfmrl)

@Stefan Mai how to handle this.
