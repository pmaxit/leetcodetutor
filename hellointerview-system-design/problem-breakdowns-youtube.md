...
By[Evan King](https://www.linkedin.com/in/evan-king-40072280/)·Published Jul 9, 2024·
...
1.  Split up the original file into segments (using a tool like [ffmpeg or similar](https://ffmpeg.org/ffmpeg-formats.html#hls-2)). These segments will be transcoded (converted from one encoding to another) and used to generated different video containers.
...
[• 6 months ago](#comment-cmgxggb7a013k07ad8nolfqvh)
...
[• 11 days ago](#comment-cmoc166vs06bu0ead0zj4i0rv)

Chunks are how the client breaks up the data for upload. The typical process is they send a request to the server that declares the size of their upload among other things. The server divides that size by the chunk size, 20 MB or so, to get the number of chunks C. Then the server requests C presigned URLs from the blob store and returns them to the user. Then the user's client handles the C uploads and sends a final message to the blob store directly, or sometimes the service, declaring "all of the uploads are done." Then the blob store makes the overall object visible. From then on anything interacting with the object sees one big object abstraction even though it was uploaded in C parts.

Note that the user controls how the video is split. And the user is not transcoding video as they upload, it's "just" a big file transfer. In general the chunks are not separately decodable to video because the byte boundaries are arbitrary. They're literally just byte ranges of the user's file.

Segments on the other hand are how the service splits one long video into small pieces that can be delivered to clients to simulate streaming. They are about 5-10 seconds long, enough to get good compression and avoid a lot of network overhead, but small enough that the download delay doesn't manifest as a ton of latency. Typically each segment is a valid container unto itself and can be played separately. That allows adaptive streaming to work easily: if the client decides to change the quality then it just downloads and plays different segments going forward. Otherwise the client would have to seek to find valid frame offsets and video metadata which would be a big annoyance.

Therefore chunks and segments are totally separate in the backend and you can't (easily at least) use chunks in lieu of segments. Also note that users can upload video to major platforms in a lot of different formats but the service transcodes them to a handful of standard formats, none of which are the upload format. This is another reason we cannot rely on the uploaded chunks as segments.
...
[• 8 months ago](#comment-cme6cvr480rxbad08swbh0bh2)
...
![Umair Shabbir](https://lh3.googleusercontent.com/a/ACg8ocKTjKiEfu_EZ3ZPtP0CCE6BoY0I4Apq3ykfCLKvNCpiyYyvFV8S=s96-c)
...
[• 1 month ago](#comment-cmmmg19dd1fwg08adtzkbi2oe)
...
[• 2 months ago](#comment-cmmbgrwke1m2u07ad0w4roshi)
