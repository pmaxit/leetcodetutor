# Design a Ride-Sharing Service Like Uber | Hello Interview System Design in a Hurry
...
[Learn This Pattern](/learn/system-design/patterns/multi-step-processes)
...
A more robust solution is to use a durable execution framework like [Temporal](https://temporal.io/) or AWS Step Functions. These systems provide built-in support for timeouts, retries, and state management in a way that survives service crashes and restarts.
...
[Next: YouTube](/learn/system-design/problem-breakdowns/youtube)
...
[• 2 years ago](#comment-cluc17rn2005i14go7ugyajs9)
...
[• 2 years ago](#comment-cluvdto5r004416rowcu8cref)
...
[• 1 year ago](#comment-cm4wc8yrb00vbu8w569gpxos8)
...
[• 1 year ago](#comment-cm4wd3m3o00x4hjxbyqf7ms70)
...
[• 4 months ago](#comment-cmj0qe2ts03mh08adv1euu5nu)

Is the problem worth solving? Let's say there are 10m drivers in the US and we store 1000 bytes per driver. That's 10Gb worth of data in RAM. A recent iPhone can fit that much. We would have to scale out Redis to allow high rates of read/writes. Even if there is stale entry for every driver in every node it's still only 10Gb per node.
...
[• 4 months ago](#comment-cmj0qfwg203qm08ad2wi2buvw)
...
[• 9 months ago](#comment-cmcrdzipu04s3ad08r0kyk1sl)

From my side, I don't think using Redis For this use case is not proper. Since we need to manage

1.  If driver is ASSIGNED or TAKING\_THE\_RIDE, then it should not exist in the geNearestDriver query.
2.  We should have TTL to only consider newest location for driver, let's say within one minute window for getNearestDriver
...
[• 1 year ago](#comment-clx7rfjx300ge9lmmo6ewmhpz)
...
[• 1 year ago](#comment-clyzn8vak001b12phrw7c0tkf)
...
[• 1 year ago](#comment-clwxom0lu00e3hrml2kh3gucu)

Hi Even, Regarding: "To solve the timeout issue, we can use a distributed lock implemented with an in-memory data store like Redis. "
...
![Ben Schmaltz](https://lh3.googleusercontent.com/a/ACg8ocIjpJqHsNZLoljsv0jNV-_9nCO7_FJHncQzcN6rNRNvH880PExe=s96-c)
...
[• 1 year ago](#comment-cm6k792cn0254oi5wdmyhu4t6)
...
[• 14 days ago](#comment-cmo8v7ljb18qu0cad54ludnju)

I think it's worth noting that distributed locks are weaker than distributed consistency in general. If you can get away with just needing locks, especially low-consequence locks like this, then usually you can get away with Redis or another solution that is much more performant per node and thus cheaper to operate.

In TM we have a flow like "ticket available" --> "ticket reserved by XYZ" --> "ticket purchased by XYZ"; we need these state transitions to be strongly consistent to avoid double-booking or a bad user experience. And to fence off "zombie actors" from degraded nodes, long GC pauses, etc. Obviously that kind of work is where a database shines.

But here Evan is suggesting locks as a rate limiter for drivers; the lock+TTL is isomorphic to a leaky bucket with capacity 1 that refills every TTL seconds. You absolutely can do that with a database. However implementing a rate limiter in memory with Redis will be much faster.

I say the locks are "low consequence" because if there is a zombie actor or the Redis node crashes we will send a second ride offer to a driver within 10 seconds. That's potentially irritating for the driver (if we have UI code that doesn't handle this edge case) but it's not a correctness problem.
...
[• 11 months ago](#comment-cmazj9wab0320ad080tnjdn3i)
...
[• 14 days ago](#comment-cmo8vqsb619h30cadcjwp2tys)

Kleppmann is absolutely correct about Redlock (or at least I trust him to be right over Redis hype!).

However Redlock versus locking as done here are two different applications of locking.

In particular Kleppmann discusses "correctness locking" versus "efficiency locking" in his book Designing Data Intensive Applications.

In correctness locking, which is what Redlock aims for, consistency failures lead to errors and serious problems like double-booking and billing without delivering. Systems that need locks to be correct should therefore use an appropriate tool designed for the job like an ACID database. Redlock was a bad idea from the start because it tries to bolt ACID properties onto Redis which is not designed for ACID. Classic round-peg-square-hole problem.

In efficiency/convenience locking, which is what we're doing here, consistency failures are just result in duplicate processing or an annoyance. In particular here if two processes think they both have the driver lock at the same time then the driver gets two ride offers in the span of 10 seconds instead of just one. If the UI code doesn't handle that gracefully for whatever reason then the driver might get a frustrating experience where their UI shows one request, then a second later paints over it with a different one. As long as that's a rare occurrence it's probably fine.

The database version has better consistency properties to be sure, but at Uber volumes, it would also require more DB nodes and maintenance effort. A decent DB node can probably do 20k transactions per second for locking, while a cheap Redis node can do 100k/second. Evan is likely implicitly trading an extra 0.05% consistency failure rate for 10x infra cost reduction for locking.
...
[• 10 months ago](#comment-cmbxcz27v095208ad556v9uyz)
...
![bristleback](https://lh3.googleusercontent.com/a/ACg8ocKEp7ccOsg067zoGOOBoFkR4MXhMXZJamdfKsFSsXLIYmMcHQ=s96-c)

bristleback

[• 3 months ago](#comment-cmk8mt4qa0b8g08adderg1phv)

How would you ensure the fare is unchanged if you don't make any writes to DB? Whenever you show a user "this ride is 5$" user expects this specific fare for some reasonable period of time.
...
[• 1 year ago](#comment-cm50q0y0g01m1ub4zoia6hzdk)
...
[• 1 year ago](#comment-clwrsqc70006zunw1re04ro05)

I came up with a little different API design. My thinking was that ride is not created at the point of fare estimation, /fare-estimate should not create a ride row and rather put the estimates in some cache with an estimate-id which can be sent to user. and the /ride/request can pass the estimate-id and then a ride row can be created. Something like POST /rides/estimate -> estimate-id POST /rides/request {estimate-id} -> ride-id
...
[• 1 year ago](#comment-clws22miw00a7rodk7z4wzoor)
...
![RK](https://lh3.googleusercontent.com/a/ACg8ocIr6z-vFuIvGVsQXdOPyl2X4dXmRYnG0R1XZ8eu_Ze9465Vfw=s96-c)
...
[• 2 months ago](#comment-cmlb6sfcn0fqj08addd9j584j)
