...
By[Evan King](https://www.linkedin.com/in/evan-king-40072280/)·Published Jul 30, 2025·
...
This handles both sustained load (the refill rate) and temporary bursts (the bucket capacity). It's also simple to implement, you just track (tokens, last\_refill\_time) per client. The challenge is choosing the right bucket size and refill rate, and handling "cold start" scenarios where idle clients start with full buckets.
...
2.  The gateway calls Redis to fetch Alice's current bucket state using HMGET alice:bucket tokens last\_refill. The HMGET command retrieves the values of multiple keys from a Redis hash. In this case, we're getting the current tokens count and the last\_refill timestamp for Alice's bucket.
...
Hot keys are often mentioned in system design discussions, and we do cover techniques for handling them in our [scaling reads pattern](/learn/system-design/patterns/scaling-reads). For rate limiting, hot keys can arise from both abusive traffic and legitimate high-volume clients.
...
The configuration service can be as simple as a database table with columns for client\_type, endpoint, requests\_per\_minute, etc. Gateways query this table on a schedule and cache the results locally.
...
[Next: FB Live Comments](/learn/system-design/problem-breakdowns/fb-live-comments)
...
[• 9 months ago](#comment-cmdub4kl40716ad07c7eppxfv)
...
[• 9 months ago](#comment-cmdugumtx08eqad075ckkgfw9)
...
[• 5 months ago](#comment-cmhsnqrzw04r408ad0jkqmlcb)
...
[• 1 month ago](#comment-cmnkexznc11620eads6mhbrxe)

There is a possibility of asking this in Coding round too, not necessarily be in LLD
...
[• 6 months ago](#comment-cmh6mq6sw055k08adw0ap9k21)
...
[• 6 months ago](#comment-cmh6new2205nn08adg2snfg3w)
...
![Ognjen Sobajic](https://lh3.googleusercontent.com/a/ACg8ocJDKsR8jDyKzm-gYy2mrtC1YuhIFm7-3xoPiYW5O8fsA-SECg=s96-c)
...
[• 7 months ago](#comment-cmfnbk5w204if07ad27gu4uc1)
...
![gaurav sharma](https://lh3.googleusercontent.com/a/ACg8ocJBu0Jnnd-dPI4modbAkhFvNCCGzScacUsD5uFQ3LCT831sCw=s96-c)
...
[• 6 months ago](#comment-cmgg46qy905bp08ado5u8frfq)
...
![nguyen bao](https://lh3.googleusercontent.com/a/ACg8ocI1pIPyLFjnTsZk2a_HbyruM-0quGuY3uY2KPU4h9RbUYrnOQ=s96-c)
...
[• 6 months ago](#comment-cmgqq0dfj05gi08adsa6um62v)
...
![Tom Tran](https://lh3.googleusercontent.com/a/ACg8ocJt1L7tLT-wJGbNurWQtPTrddGrXTO5rT83NXwgMmaGUowMa_I=s96-c)
...
[• 2 months ago• edited 2 months ago](#comment-cmlk0mljy0bi608ad9rhv4uvn)
...
\--> With the Rule only, we don't know how many requests a specific client/user have used --> When Rule changed, need to refresh the tracking number in Redis

Show more

1

![Rishabh Sharma](https://lh3.googleusercontent.com/a/ACg8ocJvSONLVsYfvuJ-RNBL9kmFDlw_54Jd4RAufO4qxBig6AXYwA=s96-c)
...
[• 5 months ago](#comment-cmi2osez205ao08adfgf70nqa)
...
[• 3 months ago](#comment-cmks0z5kc001208adz6t2k6zq)
...
![ML Consult23](https://lh3.googleusercontent.com/a/ACg8ocJ7ShB611uC5iP0p_bNQfp-cBZusqaKrnvl29hv1c2GUdSHWg=s96-c)

ML Consult23

[• 8 months ago](#comment-cme50tbn70ds7ad08thiwqkd1)
...
[• 20 days ago](#comment-cmnzj2pgs08ro0dadwljdcwyz)

Interesting. Can you discuss how DDoS protections work without using a RL system? Or maybe they use RL in some fashion but a different way?

To me it seems like DDoS and RL are basically the same problem, we want to track requests per source and avoid exceeding reasonable limits. But I'm also learning so my instinct here could be wrong.
...
[• 9 months ago](#comment-cmdvaixud0hg3ad07b3l0adis)

In this case, if the Redis master goes down, the system becomes unavailable how is the system highly available? When Redis fails, i think it takes about 15–30 seconds for the replica to be promoted to master.
...
[• 8 months ago](#comment-cme205rl8005dad085ftrgc5z)
...
![Yash Garg](https://lh3.googleusercontent.com/a/ACg8ocLsWhOJz1PoSmVmijyTZdDNjrW18pBKJJ-kef_Xa8dUihuMfC0=s96-c)

Yash Garg

[• 8 months ago](#comment-cmecgpio603nzad0743scdqbc)
...
[• 3 months ago](#comment-cmk4rarvd063y08addhxeegql)
