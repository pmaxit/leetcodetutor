# Elasticsearch Deep Dive for System Design Interviews | Hello Interview System Design in a Hurry
...
These types can be arbitrarily complex. For example, you can nest objects and arrays within your documents, use special geospatial types like geo\_point and geo\_shape, define custom analyzers, or even use embeddings for semantic search. We won't go into all of these but if you suspect you'll need something else for search, there's a solid chance Elasticsearch already has you covered.
...
*   **geo\_point**: Stores a single latitude/longitude pair. Use this for things like restaurant locations, user check-ins, or delivery addresses.
*   **geo\_shape**: Stores arbitrary geometries — polygons, lines, circles, bounding boxes. Use this for delivery zones, city boundaries, or service coverage areas.
...
Now you can run geospatial queries. The most common is geo\_distance, which finds documents within a radius of a point:
...
Under the hood, Elasticsearch uses a combination of geospatial indexing strategies — including [geohashes](https://en.wikipedia.org/wiki/Geohash), [BKD trees](https://medium.com/swlh/bkd-trees-used-in-elasticsearch-40e8afd2a1a4) (a variant of k-d trees optimized for block storage), and R-tree-like structures — to make these queries fast even across millions of documents. The geo\_point type specifically uses a BKD tree to index coordinates, which allows Elasticsearch to efficiently narrow the search space in two dimensions without the limitations of a standard B-tree index on latitude and longitude separately.
...
The search\_after parameter uses the sort values from the last result of the previous page. Here's how it works:

1.  In your initial query, you don't include the search\_after parameter.
2.  From the results of your initial query, you take the sort values of the last document.
3.  These sort values become the search\_after parameter for your next query.
...
Elasticsearch uses the point in time (PIT) API in conjunction with search\_after for cursor-based pagination:
...
3.  **For subsequent pages, add search\_after**:
...
Using PITs with search\_after provides a consistent view of the data throughout the pagination process, even if the underlying index is being updated.
...
[• 1 year ago](#comment-cm2mchnx200fbee4acs8wre0o)
...
[• 1 year ago](#comment-cm2mcvuya00gtkad7g6xl8i53)
...
[• 7 months ago](#comment-cmfk67p9t06jg07ady1v1glmn)
...
[• 7 months ago](#comment-cmfkelhqs01dl08adalnkn19l)
...
[• 7 months ago](#comment-cmfkloufk03xp08adym4afxc3)
...
[• 1 year ago](#comment-cm63vx86500mdc9i5y40p78n5)
...
![Gavin Ng](https://lh3.googleusercontent.com/a/ACg8ocIEaeWyucs3dsj4r7b8T2eaWvujUPr3G0IxqYnYKZgLiAJ16A=s96-c)

Gavin Ng

[• 1 year ago](#comment-cm7r4u7or02vx2ebzip765nzc)
...
[• 1 year ago](#comment-cm83xxcgo00cgsfnw3h6ifdbh)
...
[• 1 year ago](#comment-cm30f90tt00nxipc5f79dodsj)

I am unable to understand one point discussed in benefits of immutable architecture, Faster searches: The immutable nature allows for optimized data structures and algorithms for searching.
...
[• 7 months ago](#comment-cmgat913v04ia09adzsi1a1li)

Binary search is a good example. It requires the array to be sorted. If you keep adding elements to the array then an array + binary search is not as feasible, because you have to keep the order. In this case a binary search tree is better, but it's much more complex and comes with its own trade-offs. Making the array read-only means you only have to sort it once and you can run binary search for the queries. This is a simple example, but I believe immutability opens up similar optimizations for more advanced data structures and algorithms as well.
...
[• 1 year ago](#comment-cm3qw8r6q00ln12kc8cgkmdtv)
...
[• 2 months ago](#comment-cmlwapucc08nn08adphfw9l3r)
...
[• 5 days ago](#comment-cmokk4ek3081e08adkhaq69i0)

I think the typical pattern for CDC in general is

*   input: streaming replication slot or WAL segments, streaming replication preferred for lower latency and abstraction (so the DB can change WAL details without breaking the CDC system)
*   the node(s) tracking changes publish them to a broker system like Kinesis or Kafka or Redis Streams
*   fanout to multiple consumers is handled by the broker system

The CDC system like Debezium more or less masquerades like an async replica node to the database. It "just" receives the row level changes and pushes deltas to the brokers.
