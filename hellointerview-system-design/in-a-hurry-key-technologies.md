...
*   [Design Youtube](/learn/system-design/problem-breakdowns/youtube) -> Store videos in blob storage, store metadata in a database.
*   Design Instagram -> Store images & videos in blob storage, store metadata in a database.
*   [Design Dropbox](/learn/system-design/problem-breakdowns/dropbox) -> Store files in blob storage, store metadata in a database.
...
[Next: Common Patterns](/learn/system-design/in-a-hurry/patterns)
...
[• 1 year ago](#comment-cm8a345fx00d4853oa5z6p8lj)

Could you please do deep dives on docker and Kubernetes?

Show more

405

![Daewon Kim](https://lh3.googleusercontent.com/a/ACg8ocKHiA0pKA41OA6AM41zAIJDjlgBxg50vVg9mE5cCv0pi0QYUU4jnA=s96-c)
...
[• 4 months ago](#comment-cmj4gz0nf04df07ad1utv08iv)
...
![Prabakaran Kamaraj](https://lh3.googleusercontent.com/a/ACg8ocIGxavYlQZEz7Lpy6jzFkhiVEUPVy1jPCixDmXh1I3h1F0FIQ=s96-c)
...
[• 2 months ago](#comment-cmlzbov920fu408adnsubw716)

+1 Just for the sake of understanding

Show more

3

![Stephan Edmonson](https://lh3.googleusercontent.com/a/ACg8ocJX3Kxl2-kDvgf6JX4dkCjNggY4YZJsmzB4Jb71efeJQ2tPgR8Bhg=s96-c)

Stephan Edmonson

[• 2 months ago](#comment-cmlbftdhe01ft09adwxl7lv8d)

Are there times where you would use either for a system design interview?
...
[• 1 month ago](#comment-cmnfjwcfr0fq30dadcopctu97)

100%, especially if targeting senior+ roles. 1000% if staff+ or any kind of architect.

It's fair game for an interviewer to ask "how would you actually deploy and manage that service?" at any time. If your answer is "I would manually SSH into bare metal servers, install Nginx, then copy-paste to /dev/http" you are screwed, instant no-hire unless you are fresh out of school. The correct answer is probably to containerize and deploy on Kubernetes.

I suggest general familiarity images, pods, Deployments and StatefulSets, Services, and Controllers (especially IngressControllers and HorizontalPodAutoscalers). If you know those, as in spending a day studying K8s and following a few minikube tutorials, you will know enough to field competent answers to how to deploy and scale. Proactively suggesting things like "we can deploy this with Kubernetes and use an autoscaler to handle traffic spikes" is a strong experience signal.

By analogy: a SDI is very unlikely to require you to write plgpsql to create a specific PostgreSQL trigger function for a table you suggest. But general knowledge of transactions, triggers, and database replication strategies is essential to survive deep dives.
...
ConstantAmaranthStingray381

Premium

Premium

[• 1 month ago](#comment-cmnfmdve602vf0fadoav5fz2l)

Thank you! I am learning K8S recently and also preparing for some interviews. Your suggestions are very insightful.
...
[• 2 months ago](#comment-cmldek1kc0obi07adunvwtzx3)

Agree, not really system design interview topics. Still common in real projects though, especially outside big tech, and pretty hard to grok properly.

Show more

3

![Nitesh Gupta](https://lh3.googleusercontent.com/a/ACg8ocI7dn3kJ2QpXywdmHd_E57AnxbKdzhx3x4cjMFODcK4IjbCywU=s96-c)

Nitesh Gupta

[• 3 months ago](#comment-cmklhydhk01u507adjog2v7fb)
...
![Nathan Shangkala](https://lh3.googleusercontent.com/a/ACg8ocJ3pSqaJdYdvZzECCG84__r2kMmaXT9L41_quHQZO25tRJx300=s96-c)
...
[• 2 months ago](#comment-cmls2xlo00a3208adutbcpeyp)
...
[• 3 months ago](#comment-cml8a8lyc08na07ad0a1ntxii)
...
![Albert](https://lh3.googleusercontent.com/a/ACg8ocIuUfugn8rMZGKyZT1H619SoHimkN5itYRWi9itNC1rAZOirA=s96-c)
...
[• 21 days ago](#comment-cmnydixc90mcs07adq3qq0zyg)
...
![Amritanshu Dwivedi](https://lh3.googleusercontent.com/a/ACg8ocKvA8IxCyRk27_LoXSA5SH2YUCnjMH_6_1x3K-eVBMvvX8bIKd2=s96-c)
...
[• 1 month ago](#comment-cmn738ifp12on08ad8ckgdf4f)
...
![Phạm Ngọc Anh](https://d248djf5mc6iku.cloudfront.net/avatars/cmm4qvlgl0l0l0ead15b9y1jz-cmo1l5m52a1en0dadd1ju83ui)
...
[• 2 months ago](#comment-cmm9x1wyy064w09adw0w6qiq7)
...
[• 1 year ago](#comment-clwkrwmwv002xpc5qt5mawudf)

Thanks for this amazing write-up. I liked the Delivery Framework part but I think Core Concepts and Key Technologies part is very shallow (even for someone who is in hurry). Under Relational Databases, nowhere is mentioned about ACID properties. Please consider it as constructive feedback, I really liked this platform overall. Keep up the good work :)

Show more

28

![JAGRITI BANSAL](https://lh3.googleusercontent.com/a/ACg8ocJPcwclw3esBrqxataauPyabWkJ2Rax4Ae5d58GHm16a5kBpTKRBA=s96-c)
...
[• 1 year ago](#comment-cm7hfxrgr025fmbkqqpifve3w)

While Redlock is great for best-effort locking, it may not be suitable for highly critical systems where strong consistency is required. If data corruption is unacceptable (e.g., bank account transfers, stock trading), a consensus-based system like Zookeeper (ZAB), etcd (Raft), or Google Spanner (Paxos) is a better choice.
...
SilkyTurquoiseGoat724

Premium

Premium

[• 3 months ago• edited 3 months ago](#comment-cmka0tnry0dri08ad7975gsyh)

A Lock is temporary, so it does not require to be durable nor persistent.

Consensus is not about "protecting from data corruption", it is about handling a state across a distributed system in the presence of network partitions. If, for example, a distributed database shuts down (stops accepting writes) in case of a network partition (at least 1 node unreachable), there cannot be any of two writes | "data corruption" | an inconsistent state, even if that database has no consensus mechanism.

Redlock actually does it with a consensus mechanism. What it "does not have" is durability or persistence, which, since a lock is temporary, is not really needed: even in the case that all Redis instances explode at the same time, no other system will be able to acquire a lock, and the last system that acquired it will complete its write operation, so there is no possibility of "corruption".

The only case you can have "corruption" with Redlock is if the implementation is wrong and you have the following scenario:

*   There are 5 Redis nodes;
*   a client system acquires the lock and starts to write to the locked resource;
*   3 nodes restart, then they come back up empty;
*   another system is now able to acquire the lock, because it asks for a lock, and 3 nodes over 5 say yes (quorum of 3), and starts writing to the locked resource too;
*   the locked resource will be in an inconsistent state. But it's because of bad implementation, not because of Redis. If the quorum is "100% of the available nodes AND at least 50% + 1 node", then that scenario cannot happen.

[https://redis.io/glossary/redis-lock/](https://redis.io/glossary/redis-lock/)
...
[• 2 months ago• edited 2 months ago](#comment-cmmbn0hbx1tjb07ad9aewnj2e)

I admit that this is a thoughtful critique and it makes some valid points, but it also contains some errors and oversimplifications.

> Consensus is not about "protecting from data corruption", it is about handling a state across a distributed system in the presence of network partitions.

This statement itself is a paradox. Handling a state across a distributed system in the presence of network partitions does help protect data corruption.

> A lock doesn't need durability because it's temporary

Also a wrong statement. I expect to hold the lock for 5 minutes and because the memory crashes my lock gets lost in 1 minute. In this case, same resource can be booked twice. This is not acceptable by some systems.

Show more

0

![Ganesh R](https://lh3.googleusercontent.com/a/ACg8ocIp3ctGYTBUOO5XVK7IC7oQuLG9OIY2g4wuGbUF8m6IB5B3=s96-c)

Ganesh R

[• 1 year ago](#comment-cm7v9ochc00f912esprev5mu9)

Awesome post!

"You should be prepared to discuss how to prevent this." It would be great if you could add some common techniques used in the software industry to prevent deadlocks.
...
DelicateSilverSwan979

Premium

Premium

[• 3 months ago](#comment-cmkdzcyn301fh09adx3pyalkr)

One of the strategies to prevent deadlocks is to acquire locks on resources in same order (maybe lexicographically sorted) irrespective of whether resource A or resource B want it.

E.g. Alice wants to transfer 100$ to Bob and at the same time Bob wants to transfer 50$ to Alice.

Alice Request -> Request Lock for Alice Row and Bob Row in that order. Bob Request -> Request Lock on Bob Row and Alice Row in that order.

During simultaneous locking requests. Alice -> obtained 1st lock -> Alice Row is locked. Bob -> obtained 1st lock -> Bob row is locked.

Alice -> waiting for 2nd lock -> Bob row is already locked by Bob. Bob -> waiting for 2nd lock -> Alice row is already locked by Alice.

This is deadlock.

Now this could easliy be avoided if both have followed acquiring locks in same order.

Alice -> acquire lock on Alice row & Bob row in that order. Bob -> acquire lock on Alice row and Bob row in that order.

Now only 1 would be able to acquire the first lock. Let's say Bob acquires the 1st lock.

Bob -> acquired 1st lock -> Alice row is locked by bob. Alice -> can't acquire lock -> Wait / retry / error out.

Bob -> acquires 2nd lock -> Bob row is locked by bob. Alice -> still waiting / retry / error out.

Now Bob has locks on both whereas Alice can wait it out, retry, or error out.
...
[• 2 years ago](#comment-cltowjet9004o2oh9ko41923l)

Do you recommend memorizing these key technologies? My strategy is to know most of the key technologies by heart and study and practice the Common Problem section/answer keys. Let me know what you think!
...
[• 2 years ago](#comment-cltowpfqq002id00kmca51pum)

Memorize might be a strong word, but we recommend having a handle on these technologies and knowing some example implementations sufficiently to be able to talk about in an interview. Overall these technologies will widen your toolbox as an engineer so it's a valuable use of your time!
...
ImmediateHarlequinPeacock420

Top 5%

Top 5%

[• 1 year ago](#comment-cm19yd4nq00lwn698ajeu2j0f)

This is entire Alex Xu in one page and the best thing is that when you read it you can understand various underlying concepts that is so difficult to understand from Xu's book (especially distributed cache, locks etc). Amazing job dude!

One suggestion about DB: do we need to know about the time series dB (influx dB) where we should use it?
...
[• 1 year ago](#comment-cm19yj00w00lmd0vcfnoehpm4)

We haven't seen time series DBs in many questions out in the wild. In the places where you might use them, often the interviewer would actually prefer you design the underlying data structures directly.

The ideal case would be designing some sort of telemetry application where you have a bunch of incoming data points and need to aggregate them across arbitrary time periods.

But if I was you I'd just use postgres and hint at timescale ass an extension that makes this more performant!

Show more

12

![Eugene Scray (Jimmy)](https://lh3.googleusercontent.com/a/ACg8ocKv5sY8sLUbN2gm08i_s-gO1uicCCNqT1P_2msgWhthCL3D3o4=s96-c)

Eugene Scray (Jimmy)

[• 1 year ago](#comment-cm39ey6al00l2scsoezvgcch2)

Typo. There is an extra “s” in “as” above..
...
[• 1 year ago](#comment-cm47nkf0c02ri129iocmxtwj3)
...
FreshGrayMeadowlark251

Premium

Premium

[• 5 months ago](#comment-cmhv6l6fw01up08ad64zs3fxa)

Pretty sure that kind of questions are asked in companies such as Datadog, it would be great to have more infos about TimeSeries DBs or at least example about Telemetry System Designs.
...
[• 3 months ago](#comment-cmka1476s0dx208adb0d79yk0)

Also in Ad Clicks Aggregation a Time Series DB could be a good option.
...
[• 1 year ago](#comment-cm1a1sf9z00pr11v2c2nc9m8b)

Thanks I was looking at Google Bigtable and that also has timestamp as another dimension. For example it is a key value pair and then you have the timestamp for every value. Do you think using Bigtable in that kind of application is a good choice.

Now since you mentioned about the data structure for the time series dB can we just use defaultdict(lambda: defaultdict(list)) as the basic data structure . I know it is more of a coding question than the system design you can ignore it.

Thanks for these informations. Super helpful. cannot thank you more.
...
[• 9 months ago](#comment-cmdsjr8br08bfad07ri0370mk)

influxDB would be a great choice for designing an events monitoring dashboard of some sort. Kubernetes internally uses it to capture infra level events.
