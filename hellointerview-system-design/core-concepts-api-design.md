...
In system design interviews, you'll want to define how clients interact with your system as part of the [API step](/learn/system-design/in-a-hurry/delivery#api-or-system-interface-5-minutes) in the [Delivery framework](/learn/system-design/in-a-hurry/delivery).
...
The foundation of good REST API design is identifying your resources correctly. If you've followed the [delivery framework](/learn/system-design/in-a-hurry/delivery), resources are just your [core entities](/learn/system-design/in-a-hurry/delivery#core-entities-2-minutes).
...
When handling relationships between resources, you have two main approaches. You can nest resources when there's a clear parent-child relationship, like /events/{id}/tickets for all tickets belonging to a specific event. Alternatively, you can keep resources flat and use query parameters for filtering, like /tickets?event\_id=123.

The key difference is whether the relationship is required or optional. Use path parameters (or nested resources) when the value is required - like /events/{id}/tickets where you always need to specify which event's tickets you want. Use query parameters when the filter is optional - like /tickets?event\_id=123&section=VIP where you might want all tickets, or tickets filtered by event, or tickets filtered by both event and section.
...
For our Ticketmaster example, authentication might verify that the request comes from user "[john@example.com](mailto:john@example.com)", while authorization checks if John is allowed to cancel the specific booking he's trying to cancel (he should only be able to cancel his own bookings, not everyone's).
...
![shrewquest](https://lh3.googleusercontent.com/a/ACg8ocJ6kUUiIbf2EmoKRktiyTgn-lEIi_4UeiupQKdKIdhm76DAW7o=s96-c)
...
[• 9 months ago](#comment-cmdh0cktm03lbad077s8277j3)

I suppose API design can be its own interview though I'm not aware of any companies that ask it though it's useful on the job. Some more things to consider would be: forward/backward compatibility (things like enum instead of bool field to be extensible, removing fields breaking compatibility), exceptions (clarifying retryable vs non-retryable, http codes, security considerations in the err msg, clarity on what failed & what didn't for batch APIs), consistent filtering, avoiding nested pagination etc, perhaps more protocol specific and security considerations
...
[• 9 months ago](#comment-cmdhix16h06m5ad078c9b1qr6)

Yah, you're right to point out that that is a different interview type and this article won't give you everything thing you need for it, but you've added a great list here!
...
DramaticAmaranthAlligator843

Premium

Premium

[• 4 months ago](#comment-cmj2y1aff04yf08ad7re9nucb)

A friend in Atlassian was grilled for an hour on API design in their system design round!

Show more

3

![안광훈](https://lh3.googleusercontent.com/a/ACg8ocJeZUFE5tvJGiAi_KD2e-tzoNKCZR5pd_Mu2JNT6TM3D7kyFA=s96-c)
...
[• 1 day ago](#comment-cmoqcbn4t1e680eada1s3qsr9)

Scale AI interviewer spent a lot of time on API design, and asked me to walk him through how to use presigned URL at implementation level. It looks quite heavy dependent on interviwer.
...
[• 9 months ago](#comment-cmdhpk7lr00dfad081r2h4nd2)

This is fantastic! Thank you.

Is there anything in the pipeline in regards to data modelling and setting up tables for optimal query / access patterns? It's the part I struggle with sometimes when doing guided system design. I feel like just listing the entities is sometimes not enough for interviews.. they expect a somewhat in-depth data model
...
[• 9 months ago](#comment-cmdhprsnq00dsad0810oscv21)

There isn't, but this sounds like a very reasonable "core concept." Will add it to the list! But no guarantee on timing.
...
[• 7 months ago](#comment-cmg8nh76c00iw08adfewt2h04)

He kept his word, and data modeling is available. Thanks!
...
[• 7 months ago](#comment-cmg8niowt00ja08adb6zybnfe)
...
[• 9 months ago](#comment-cmdqbrfbf06rwad077eybpkw2)

I agree - It has become a very important part of system design these days and comes in at an earlier phase of the interview - and if you get stuck then you know it's done.
...
[• 9 months ago](#comment-cmdmgh8i3068gad08qbf5a2ia)

I know it is a bit picky, but it always gets to me when I read "PUT is idempotent". It may give the idea that when using the PUT verb by itself, the call is idempotent, when in reality it is up to the service that handles that request to do it idempotently. Rather saying that the standard or RESTful specification says that PUT method should be idempontent by implementation sound a little bit more precise to me, though I may be missing something.
...
[• 9 months ago](#comment-cmdmgico806wbad08qvob038v)

Correct. The verb itself does nothing. It's the expectation that, for it to be RESTful, its implementation is idempotent.

Show more

6

![Luis Alberto Beltrán Pérez](https://lh3.googleusercontent.com/a/ACg8ocIaSA_1cLSHmOXSi481jNSShlTtZgiihR8Ewm_Zu5KhKcnXgfrw=s96-c)

Luis Alberto Beltrán Pérez

Premium

Premium

[• 3 months ago• edited 3 months ago](#comment-cmkhtgwn405w909ad3lp36fow)

Technically this applies to the other HTTP methods, no? You could have the GET API do something internally to alter the final state. So I'd say REST aims for all methods except DELETE to be idempotent.

Show more

3

![Siddhartha Arora](https://lh3.googleusercontent.com/a/ACg8ocJn5fdl4DbYs1QLZMbaYLCqJJJGkANyytmBFq7EyrwO2FgS13pdoA=s96-c)
...
[• 2 months ago](#comment-cmlmeapy806mq09ad0l1iiix7)

I think its good to mention that the "N+1" queries problem is not GraphQL specific and REST does not always solve it. Its just that GraphQL makes it easier to create "N+1" problems as resolvers are composed dynamically and response shape is controlled by the clients while in REST we generally write optimized queries and the response shape is controlled by the server.
...
[• 9 months ago](#comment-cmdhmxfl007poad09e1kkvl9t)

from gemini , but I trust hello interview more As an API design expert, let's break down the two options for a Ticketmaster-like booking API and determine which is "more correct" and why.

Analysis of the Options:

POST /bookings/:eventId -> bookingId

Interpretation: This suggests creating a "booking" at the top level of your API, with the specific event ID being passed as part of the request body (or perhaps as a query parameter, though the URL structure implies a path parameter). The path parameter :eventId here is a bit misleading if the primary resource is bookings.

RESTful Perspective: If bookings is the primary resource being created, the more typical RESTful approach would be POST /bookings. The eventId would then be part of the request payload, identifying what is being booked.

Clarity: The path POST /bookings/:eventId feels a bit off. It implies you're acting on an eventId to create a booking, but bookings is the collection being modified.

POST /events/:eventId/bookings -> bookingId

Interpretation: This clearly indicates that you are creating a "booking" that is subordinate to a specific "event." You are performing an action on the bookings collection within the context of a particular eventId.

RESTful Perspective: This is the more idiomatic and RESTful approach. It represents a sub-resource collection.

events is a collection of event resources.

events/:eventId is a specific event resource.

events/:eventId/bookings is a collection of bookings associated with that specific event.

POSTing to this path means you are creating a new booking within that event's booking collection.

Clarity and Discoverability: This structure makes it immediately clear that bookings are tied to events. If someone is exploring your API, they can easily understand the relationship.

Conclusion: POST /events/:eventId/bookings -> bookingId is the more correct and preferred API design.
...
PoliteVioletMastodon483

Top 5%

Top 5%

[• 9 months ago](#comment-cmdxzt14v02lbad08st4ujf5q)

POST /bookings/:eventId is clearly a non-starter. The resource should contain its own resource id, not other resources' id. That being said, valid REST API could have: /v1/events/:eventId/bookings - for creating booking - returns a bookId. /v1/bookings/:bookingId - for viewing bookings

Show more

3

![Kartikeya Srivastava](https://lh3.googleusercontent.com/a/ACg8ocLD_5zWSBw34IGZyNW4ETqwCu7cq5aW8devXxNhz72OusV-zQ=s96-c)
...
[• 1 month ago](#comment-cmmq2au6703z508adqmkdz380)

Thanks for this suggestion. It makes more sense when I look deeply and it overall creates a structured way of defining our endpoints
