...
While anything from MySQL to DynamoDB would be fine choices (just needs ACID properties), we'll opt for [PostgreSQL](/learn/system-design/deep-dives/postgresql). Additionally, we need to implement proper isolation levels and either row-level locking or Optimistic Concurrency Control (OCC) to fully prevent double bookings. More on this later in our deep dives.
...
[Next: FB News Feed](/learn/system-design/problem-breakdowns/fb-news-feed)
...
![Mallik Hassan](https://lh3.googleusercontent.com/a/ACg8ocIfMCXbUgEeHnxGmAIg0B0OVHbZIFMWtYFkr6QaAGQ8XwD8DosDsg=s96-c)
...
[• 8 months ago](#comment-cmebtf1yx01kkad08xm2xdojc)
...
[• 2 years ago](#comment-clt872eir0001n1skmnnpmtfk)
...
[• 2 years ago](#comment-clt87r0tb000b53e5dam2h6we)
...
[• 2 years ago](#comment-cltvufytc00gn3d16kn45la4i)
...
[• 1 year ago](#comment-clw6zdfr0003s13crux60ixgy)
...
[• 1 year ago](#comment-clw74cjyo000tgexsaecr4tja)
...
[• 10 months ago](#comment-cmch6a06204cmad08omcm3odw)
...
![Paroksh Saxena](https://lh3.googleusercontent.com/a/ACg8ocKDd_e0Hke6vWSqvGAea4ORi3civdKTnUzMq4Rf06tQ1HJfM0Tz=s96-c)
...
[• 14 days ago](#comment-cmo8h3b6t0upt08aduj4tsrpo)

We need transactions here, so Postgresql or DynamoDB ( with ACID ) would work Since we also need relations between event / ticket / booking, relational database makes sense
...
[• 1 year ago](#comment-cm94b4hsz00pdad08jp9hchho)
...
[• 6 months ago](#comment-cmgwde3ey01tp09adh9wrqx43)

Redlock is a client side algorithm to acquire and release lock and there is no leader election happen in the naive setup which requires majority node responses for acquiring the lock. Due to distributed leaderless setup where all the nodes act as primary /master, availability could be tolerated up to certain node failures, for e.g. with 3 it is 1 and for 5 it is 2.

Show more

0

![Yasir Mukhtar](https://lh3.googleusercontent.com/a/ACg8ocK4N5zdZ3JTi-rLKISvYMMxxfVv8m-bkNWXeMAC5AmTnu3CosfY=s96-c)
...
[• 4 months ago](#comment-cmjpc17pn06jj08adfz72vhk7)

You can reserve the tickets in DyanmoDB Lock table with ttl.
...
[• 1 year ago](#comment-cm7ohocha000mrpj4fe8x0iks)
...
[• 1 year ago](#comment-cm8c25jch006o8pgzmgum33i6)
...
[• 1 year ago](#comment-clwo16tk0001gp3qlyejejzru)
...
7 replies hidden which refer to old versions of the article. [Expand them.](#)
...
[• 2 years ago](#comment-cluf0uiuz003avjkia9xiu7c5)

There is a paper on the Redlock algorithm which Redis discusses how to mitigate the single point of failure, and how to work around the issue where Redis replication being async: [https://redis.io/docs/manual/patterns/distributed-locks/](https://redis.io/docs/manual/patterns/distributed-locks/)
...
[• 1 year ago](#comment-cm7oie0r300152ebzuv7x19td)
...
[• 6 months ago](#comment-cmgkgomf701am08adfdzbt77a)

IMO, it will involve master-slave replication and due to replication lag and when leader fails the lock might not replicated to new leader elected from slaves . Correct me if im wrong here
...
[• 1 year ago](#comment-cm6trjrwy004m7oi9qe03erw0)
...
![Syed Bilal](https://lh3.googleusercontent.com/a/ACg8ocLMSYKbl0ySUcTbPVMoFdPuFfcI7r6i_b4uXg3alpnMjj29qopW=s96-c)
...
[• 1 year ago](#comment-clywxugen00cbul9cmaawi1h9)

There are some really good discussions here down in the discussion section.

I would recommend you to add feature to pin the favourite comments and beside anonymous button, a filter feature can be added to show only favourite comments for a particular user. It will help you to pin some of the important comments or mark it for future easy reference instead of reading all the threads to search for that comment.
...
[• 1 year ago](#comment-clyx2ftwt00ditois480jn6q1)

Agreed :) I will add it to the list and hopefully get to it soon. The comment became a bit unwieldy in length.
...
[• 1 year ago](#comment-clyx5i9e000d8ul9cx8ak9mkb)
...
LimitedEmeraldKoi758

[• 2 years ago](#comment-clvifm47g006ffnqriwrjhl29)

Wouldn't we want the virtual waiting queue to queue users accessing the Event CRUD service? Not the booking service?

The booking service has two APIs (reserve and confirm) but don't we want to bar users from seeing the event with live updates to the seatmap first (i.e. /GET event/:event\_id)?
...
[• 2 years ago](#comment-clvigwr9h0082w2xidke17bhn)
...
MeaningfulGreenFerret245

[• 11 months ago](#comment-cmbbt1ah101bnad07qbtt6jwb)

hi Evan i also have same question, so for users to see the updated seatmap, can we use server sent events ,where the seat map will be updated, if other users are selecting the seats. also is reserve and confirm api called at once ?
...
[• 2 years ago](#comment-clvigd6cs008arejyrtu6qxdp)

Also a HUGE thank you for putting this resource together. The videos in particular are very useful :)
...
![Castulo J. Martinez](https://lh3.googleusercontent.com/a/ACg8ocJhLtMoci3SLvrpMtXfer-A-UYTwTCz6K5SeKDm4xgL7JjYXYcl9w=s96-c)

Castulo J. Martinez

[• 1 year ago](#comment-clvzrv8h800098zk3bma4imnq)

When designing an application using a micro-service architecture, when is it better to access directly a database shared by another service so both services end up using the same database, and when is preferable to call the other service to request this info so only one service can access the database and pass the requested information to the service that requested it?
...
[• 1 year ago](#comment-clvzs1d2f000ajhytspoq67m7)

There's options here. A "true" microservice architecture will have each service own its own data - but this creates a bunch of new problems around data integrity and synchronization. Generally speaking the larger the org the more likely you want to move toward data isolations to prevent issues.

If this was in an interview I'd discuss the tradeoffs with my interviewer. "I think for our purposes I'll use a single database here but if this was a real project I'd evaluate the tradeoff of complexity for extensibility." Prepare to explain yourself :)
...
[• 28 days ago](#comment-cmnntis8l04kb08adgiz8bunn)

Common database: consistency and coordination. For consistency the locking here is a great example. Having 2+ databases with seat information becomes a nightmare when you need to synchronize their state with respect to seat statuses. Avoid distributed atomic operations (e.g. DB transactions) at all costs. For coordination if service A needs to exchange information with service B then it's often easiest to have a database they store. For example they can both observe the same DB state instead of having to effectively do async replication and change data capture at the service level.

If you run a small project then a single DB can help as well as a sin of convenience. Hosting one database is easier and cheaper than maintaining a database per service across a dozen services.

Per-service database: when each service is responsible for a different view of the data. And especially if the data is unrelated. For example in this problem we could have separate databases for event metadata (the artist, even name, etc.) and event seating (per-seat statuses, locking, etc.). Real TicketMaster seems to have this distinction based on my poking around with devtools. The access patterns are different for each kind of data so it stands to reason we may want to scale and shard them differently. If the data is disjoint (like seating versus metrics) then separate databases per service makes even more sense because it decouples them, allowing e.g. ACID/SQL for seat statuses and eventually consistent document stores for metadata.
