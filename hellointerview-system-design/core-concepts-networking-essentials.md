...
If you've followed our [Delivery Framework](/learn/system-design/in-a-hurry/delivery), your Core Entities will oftentimes map directly to the resources in your API. Bonus!
...
We touch on this pattern in greater detail in our [Realtime Updates](/learn/system-design/deep-dives/realtime-updates) deep dive, which also covers the server-side implications of an SSE implementation.
...
In practice, most collaborative editors _don't_ require scaling to thousands of clients. Additionally, you often need a central server anyways to store the document and coordinate between clients. That's why we're using Websockets in our [Google Docs problem breakdown](/learn/system-design/problem-breakdowns/google-docs). But there is an alternative to use WebRTC and CRDTs (Conflict-free Replicated Data Types) to achieve a truly peer-to-peer experience.
...
[• 1 year ago](#comment-cm7pbek1f00u3jtnq4iatuesg)
...
![haha hehe](https://lh3.googleusercontent.com/a/ACg8ocKMe_9w7GVji8xV9y_sMgZEQkgpybrQRkonkKBsjuy4=s96-c)

haha hehe

[• 1 year ago](#comment-cm8kh8axz0069139lv01ujywn)

A lot of other blogs/vlogs mention that for HTTPS/Websocket connections we should use Layer 7 load balancers. Moreover, for persistent connections or sticky sessions too, they suggest using Layer 7 load balancers as you can inspect cookies, etc. But you suggest that we use Layer 4 Load balancers. Can you please throw some more light on your reasoning?

Show more

8

![Kaushik Nath](https://lh3.googleusercontent.com/a/ACg8ocKAoeeQgcmI5nAcRlQNP_eTQSHB1sAbBQxik68pWjtp2pvijZXW=s96-c)
...
[• 5 months ago](#comment-cmiatxcgx04tw08adzpcdtsd2)
...
![Mamuni Jubuli](https://lh3.googleusercontent.com/a/ACg8ocIMDsIUhXkgvM94nUEdqEXfSZptnHlSWwBdNzZgw3bnxSzHdQ=s96-c)

Mamuni Jubuli

[• 3 months ago](#comment-cmk2ylef806oq08adllpv0l7u)
...
I think, Yes, we will have 2 websocket connections. One is between the user and load balancer and another between load balancer and server. So that means the load balancer also has to understand the websocket protocol.

Since you have worked in Application gateway and have worked on websockets, could you please clear my doubts ?
...
[• 11 months ago](#comment-cman921bb01fxad08v3qnxdnz)
...
[• 1 year ago](#comment-cm9yzh8rw01d1ad08ksxufyqv)
...
![Alex A](https://lh3.googleusercontent.com/a/ACg8ocIWl_xMmR1qJlihyuFGGm4T3SiiqLAS0GSrvKOkkE45ykPF0Q=s96-c)
...
[• 5 months ago](#comment-cmi19cny0038m08adlatzs5n6)
...
[• 3 months ago](#comment-cmk2z3rey06xt08adz6ctwbgt)
...
[• 3 months ago• edited 3 months ago](#comment-cmk31d7ph00qw08adwr75ocpb)
...
![Shanshan Zhu](https://lh3.googleusercontent.com/a/ACg8ocLMfbECrK8cYByaVGw804tGbGVGXA6rg-UCRFiEYZv-qeCBSls=s96-c)
...
[• 25 days ago](#comment-cmnsk0wn20fjb08ad3c90gmrg)

I'm also confused by the author's recommendation of using L4 versus L7 for websocket load balancing.

Alex is correct that the load balancer distribute the connection, not the message after connection is established. Hence when WS failed, client side need to reconnect, and it is usually not the job of LB to reconnect.

Therefore, it is simplified for LB. Envoy, AWS ALB supports WS Load balancer on the L7 layer, rather than L4. B/c it provides richer features like routing based on url path, hostname, cookies, or other application-layer detail.
...
[• 7 months ago](#comment-cmfueyayz02lk08ad3bt304kv)

**TCP (Transmission Control Protocol)** Type: Connection-oriented, reliable protocol How it works:Creates a connection between client and server (three-way handshake) Ensures reliable, ordered, error-checked delivery of data packets Use cases: Web browsing (HTTP/HTTPS), Email (SMTP), File transfer (FTP) Pros: Reliability, order, error checking Cons: Slightly slower due to extra overhead

**2\. UDP (User Datagram Protocol)** Type: Connectionless, faster but less reliable protocol How it works:Sends packets called datagrams without setting up a connection No guarantee packets will arrive or arrive in order Use cases: Video streaming, online gaming, VoIP (real-time communication) Pros: Low latency, faster transmission Cons: No reliability, no ordering, no congestion control

**3\. QUIC (Quick UDP Internet Connections)** Type: Modern transport protocol built on UDP How it works:Developed by Google; now basis for HTTP/3 Uses UDP’s speed but adds features like: Reliability & encryption (like TCP + TLS combined) Faster connection establishment (0-RTT) Multiplexing streams → prevents head-of-line blocking issues in TCP Use cases: YouTube, Google services, HTTP/3 web browsing Pros: Fast, secure, reliable, low latency

Cons: Newer → not supported everywhere yet
...
GoodTurquoiseKoala188

Premium

Premium

[• 1 year ago](#comment-cm88wuz4q00gl3rzaek7bdwgz)

Thank you for the detailed right up! I have a question about auth, SSL termination when it comes to L4 load balancer and using websockets. You mention that it is recommended to use L4 load balancer when using websockets. But then as a result, we're not able to also use API Gateway for authentication or SSL termination because if we do so it will no longer be TCP pass through and defeats the purpose of using L4 right? In that case if we are only using L4, would we just have to use another service on the server side to handle auth and SSL termination rather than rely on API Gateway?
...
[• 9 months ago](#comment-cmducvpls07jfad08d36kx1db)
...
NuclearVioletWoodpecker390

Premium

Premium

[• 9 months ago](#comment-cmd88uhqh02odad081sq07vg4)
...
![Aparna Rajan](https://lh3.googleusercontent.com/a/ACg8ocKitBTLl03jkVAd4UNUYvpIGMHqe-myVSBED6J0mt45OOP7iSP7=s96-c)
...
[• 11 months ago](#comment-cmbfzm19100qqad088o7xebka)
...
[• 1 year ago](#comment-cm7qtlphh02cfpqpvptljd5vj)
