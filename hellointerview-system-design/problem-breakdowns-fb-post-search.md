# Design Facebook's Post Search | Hello Interview System Design in a Hurry
...
By[Stefan Mai](https://www.linkedin.com/in/stefanmai/)·Published Jul 4, 2024·
...
[Next: YouTube Top K](/learn/system-design/problem-breakdowns/top-k)
...
![Eugene Myasyshchev](https://lh3.googleusercontent.com/a/ACg8ocJs-nRTPRFN1MYQY8q86WTZzz3XUgZb6TUulsWSxA9KXlfT1P_vBA=s96-c)
...
[• 1 year ago](#comment-cm21w3jsc00dcniqrkyumrtx6)
...
[• 4 months ago](#comment-cmjd4d94w04aa08adsuerddlc)
...
[• 1 year ago](#comment-cm2jdb4wb0298wxs6nec4v4jv)

I think its assuming the postId is 8bytes(unsigned 64bits i.e. 2^64) and the redis is sharded by a hash of the keywords (using a 64bits non-cryptographic hash). Yes, you're right the length of the Redis sorted list of post\_ids need to be capped and less liked post can be stashed away in cold storage in small chunks identified by range keys for easier retrieval and update when needed.
...
[• 1 year ago](#comment-cm51a8su901tir5q6whfgy1ul)

In no way you can clear the interview with this design. You can't put all the index into memory, it's not practical in both ever increasing storage and astronomical cost. Also it doesn't make sense. You at least need a layer architecture by time. Say day / week / month / year index layers. A day's data in many cases give you more than 1000 posts, if not enough the pagination can always prefetch a week's post in the background while user scrolling. Day index can fit into memory, all the others should be in a read optimized DB. Likes compares to index is exponentially smaller, it's a counter per post v.s. multiple terms per post. Even if you store the like userId in a list, it's still much smaller. You can easily put a whole year's worth of like data into memory, and use it during ranking not querying.
...
[• 1 year ago](#comment-cm51lj3rp0287ub4z4c1iuwy4)
...
[• 1 year ago](#comment-cm51ns3em027tmqq6j20tuvls)
...
[• 1 year ago](#comment-cm51nvwkp027wmqq6slpp8yf0)
...
[• 1 year ago](#comment-cm51o3ljw028h13vuuh150lfg)
...
[• 11 months ago](#comment-cmams30q800r1ad08hbf6opkj)
...
[• 11 months ago](#comment-cmams98bp00x4ad08woodtv4h)

We are [limiting the size of each index item](https://www.hellointerview.com/learn/system-design/problem-breakdowns/fb-post-search#4-how-can-we-optimize-storage-of-our-system).
...
[• 1 year ago](#comment-cm1axkav0005o11ugvrezzg88)
...
[• 1 year ago](#comment-cm1ay45mt006ru1f4j88vjq7y)
...
[• 1 year ago](#comment-cm2vgm5s000jk4rx9lye2koqf)
...
[• 1 year ago](#comment-cm7wxosqo002ttvyj8hklwv7w)
...
[• 1 month ago](#comment-cmn5li3yj05yv08ad0x4uzglm)
...
1.  consumer polls the broker, broker responds with <= batchSize messages since the consumer's last committed offset
2.  hit backend APIs for incrementing like count, updating indices every time or in logarithmic steps
...
*   update a likes table to add the (user\_id, post\_id) where user liked the post
*   update the posts table, setting likes = likes + 1 if and only if (user\_id, post\_id) was added

From some reading elsewhere a common pattern seems to be

*   store (user\_id, post\_id) interactions in a strongly consistent way, full accuracy
...
[• 1 year ago](#comment-cm6gb0b4e073hsn4h4fbhrgeq)
...
[• 12 days ago](#comment-cmobcb6qn0b3908adcohcgw4b)

I think what we store in the index memory is mapping of words to post ids. We still need to get the actual posts from post service. But the cache used here is to cache result of an entire query (including the post details), so we skip the call to Post service here.
...
[• 1 year ago](#comment-cm6kk0o5t02k011hvstcsu6ck)
...
[• 9 months ago](#comment-cmcz46lzg0alaad082p3r7se2)
